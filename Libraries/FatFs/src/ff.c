...
/*-----------------------------------------------------------------------*/
/* Unicode - Local code bidirectional converter                                                      */
/*-----------------------------------------------------------------------*/
...
static
int cmp_sfn (const void* dst, const void* src, UINT cnt) {
	...
	do {
		...
		if (c == DDEM || ((a & AM_VOL) && a != AM_LFN)) {	/* An entry without valid data */
			...
			if (a == AM_LFN) {			/* An LFN entry is found */
				...
				if (!(dp->fn[NSFLAG] & NS_LOSS) && !cmp_sfn(dir, dp->fn, 11)) break;	/* SFN matched? */
				ord = 0xFF; dp->lfn_idx = 0xFFFF;	/* Reset LFN sequence */
			}
		}
#else		/* Non LFN configuration */
		if (!(dir[DIR_Attr] & AM_VOL) && !cmp_sfn(dir, dp->fn, 11)) /* Is it a valid entry? */
			break;
#endif
