...
static const HID_Report_ItemTypedef prop_wheel_dir =
{
  (uint8_t *)(void *)mouse_report_data + 6, /*data*/
  8,     /*size*/
  0,     /*shift*/
  0,     /*count (only for array items)*/
  1,     /*signed?*/
  0,     /*min value read can return*/
  0x00FF,/*max value read can return*/
  0,     /*min vale device can report*/
  0xFFFF,/*max value device can report*/
  1      /*resolution*/
};
...
static USBH_StatusTypeDef USBH_HID_MouseDecode(USBH_HandleTypeDef *phost)
{
  ...
  if (USBH_HID_FifoRead(&HID_Handle->fifo, &mouse_report_data, HID_Handle->length) ==  HID_Handle->length)
  {
    ...
    mouse_info.wheel_dir = (uint8_t)HID_ReadItem((HID_Report_ItemTypedef *) &prop_wheel_dir, 0U);
